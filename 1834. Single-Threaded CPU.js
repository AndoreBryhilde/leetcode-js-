...
var getOrder = function(tasks) {
    ...
    let ans=[];
    let que=new Heap((a,b)=>a[1]!==b[1]?a[1]<b[1]:a[2]<b[2],[]);
    let time=tasks[tasks.length-1][0];
    while(tasks.length||que.length){
        while(tasks.length&&time>=tasks[tasks.length-1][0]){que.insert(tasks.pop())}
        if(que.length){
            let [t,c,i]=que.remove();
            time+=c;
            ans.push(i);}
        else{
            if(tasks.length){time=tasks[tasks.length-1][0]}
        }
    }
    return ans;
};
